 ...
 * what platform/format) and categorization (pillar), and the post's own content (title, hook, caption,
 * hashtags, call to action) — used to reschedule/recategorize
...
export function updateContentIdea(id: number, data: ContentIdeaUpdate): ContentIdea | null {
  ...
  const merged: ContentIdeaRow = { ...existing, ...rest, ...(hashtags !== undefined ? { hashtags: JSON.stringify(hashtags) } : {}) };
  ...
  db.prepare(
    `UPDATE content_ideas SET target_date = ?, suggested_time = ?, platform = ?, format = ?, status = ?, pillar = ?, product = ?, hook = ?, caption = ?, hashtags = ?, cta = ? WHERE id = ?`,
  ).run(
    merged.target_date,
    suggestedTime,
    merged.platform,
