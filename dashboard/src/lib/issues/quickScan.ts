...
import { DEFAULT_QUICK_SCAN_PAGES, MOBILE_VIEWPORT, DESKTOP_VIEWPORT, SITE_URLS, type QuickScanPage } from "./config";
import { fetchOpenQuotations } from "../odoo/quotations";
import { generateOdooOrderAlerts } from "../odoo/orderAlerts";

/**
 * Live, on-demand checks: Playwright against both public sites, plus an
 * Odoo open-quotation scan for order alerts. Read-only: navigates and
 * inspects only, never fills in forms, never adds to cart for real, never
 * writes to Odoo.
 */

const MAX_LINKS_TO_CHECK = 30;
const NAV_TIMEOUT_MS = 15_000;
// 6 page checks + retail link crawl + wholesale link crawl + 1 Odoo order-alert pass.
const TOTAL_CHECK_STEPS = DEFAULT_QUICK_SCAN_PAGES.length + 3;
...
export function startQuickScan(): number {
  ...
  const result = db.prepare(`INSERT INTO scans (scan_type, status, total_checks) VALUES ('quick', 'running', ?)`).run(TOTAL_CHECK_STEPS);
...
async function runQuickScan(scanId: number): Promise<void> {
  ...
  const recordCheck = (result: CheckResult) => {
    db.prepare(
      `INSERT INTO checks (scan_id, check_type, category, site, page, label, status, details) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    ).run(scanId, result.check_type, result.category, result.site, result.page, result.label, result.status, JSON.stringify(result.details ?? {}));
  ...
  try {
    ...
    const retailLinks = await collectInternalLinks(browser, SITE_URLS.retail);
    await checkLinks(browser, retailLinks, "retail", recordCheck);
    tick("Broken link scan — Shopify");

    const wholesaleLinks = await collectInternalLinks(browser, SITE_URLS.wholesale);
    await checkLinks(browser, wholesaleLinks, "wholesale", recordCheck);
    tick("Broken page scan — Odoo");

    try {
      const openQuotations = await fetchOpenQuotations();
      generateOdooOrderAlerts(openQuotations);
    } catch (err) {
      // Odoo unreachable shouldn't fail the whole check — the Playwright
      // results above are still valid and already recorded.
      console.error("[website-health] Odoo order-alert check failed:", err);
    }
    tick("Odoo order-alert scan");
...
async function checkPage(browser: Browser, target: QuickScanPage, recordCheck: (r: CheckResult) => void): Promise<void> {
...
async function checkLinks(browser: Browser, links: string[], site: "retail" | "wholesale", recordCheck: (r: CheckResult) => void): Promise<void> {
  const context = await browser.newContext();
  try {
    for (const link of links) {
      const label = `${site === "wholesale" ? "Wholesale" : "Internal"} link — ${(() => {
        try {
      ...
      try {
        const response = await context.request.get(link, { timeout: 10_000, maxRedirects: 0, failOnStatusCode: false });
        const status = response.status();
        const kind = status >= 400 ? "fail" : status >= 300 ? "warn" : "pass";
        recordCheck({ check_type: "link", category: "navigation", site, page: link, label, status: kind, details: { destination: link, httpStatus: status } });
      } catch (err) {
        recordCheck({
          check_type: "link",
          category: "navigation",
...
/** Rolls failing/warning checks from one scan into deduped, tracked issues. Exported for direct DB-level testing. */
export function generateIssuesFromScan(scanId: number): void {
  const db = getHealthDb();
  const problemChecks = db.prepare(`SELECT * FROM checks WHERE scan_id = ? AND status != 'pass'`).all(scanId) as Array<{
    check_type: string;
    category: string;
    site: string;
    page: string | null;
  ...
  const upsertIssue = db.prepare(`
    ...
    VALUES (@fingerprint, @severity, @category, @site, @page, @title, @description, @evidence, 'quick_scan', datetime('now'), 1)
  ...
  const cleanChecks = db.prepare(`SELECT * FROM checks WHERE scan_id = ? AND status = 'pass'`).all(scanId) as Array<{
    category: string;
    page: string | null;
    label: string;
  }>;
  const resolveIssue = db.prepare(`UPDATE issues SET status = 'resolved' WHERE fingerprint = ? AND status != 'resolved' AND source = 'quick_scan'`);
...
export function getScanStatus(id: number): ScanStatus | null {
  return (getHealthDb().prepare("SELECT * FROM scans WHERE id = ?").get(id) as ScanStatus | undefined) ?? null;
}

export function getScanChecks(id: number) {
  return getHealthDb().prepare("SELECT * FROM checks WHERE scan_id = ? ORDER BY id ASC").all(id);
}

export function getLatestScan(scanType: "quick" | "full" = "quick"): ScanStatus | null {
  return (
    (getHealthDb().prepare("SELECT * FROM scans WHERE scan_type = ? ORDER BY started_at DESC LIMIT 1").get(scanType) as
      | ScanStatus
      | undefined) ?? null
  );
}
